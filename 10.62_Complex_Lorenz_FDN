...
// GUI of the System
delEQ1 = hslider("Delay 1[unit:milliseconds]", 
0, 0, 1000, 0.1)*ma.SR/1000.0;
delEQ2 = hslider("Delay 2[unit:milliseconds]", 
0, 0, 1000, 0.1)*ma.SR/1000.0;
delEQ3 = hslider("Delay 3[unit:milliseconds]", 
0, 0, 1000, 0.1)*ma.SR/1000.0;
Treshold = hslider("Saturation limit", 4, 1, 1024, .000001);
CFblock = hslider("DC Blocker FCut[unit:Hz]", 20, ma.EPSILON, 100, .001);
CF = hslider("F-Cut[unit:Hz]", 1000, 20, 24000 - 20, .001);
...
costrained(Delsamps,Treshold,CFblock, K, Q, CF, 
Glp , Ghp , Gbp, Gnotch, Gapf, Gubp, Gpeak, Gbshelf, x) 
= routingout
with{
...
routingout = 
    ...
    ) : vardelay(196000,Delsamps,ma.SR/1000.0,0);
};


// (lorenz eq. ---> costrained) x3
lorenz(x0,y0,z0,sigma,rho,beta,dt,Dt1,Dt2,Dt3,Treshold,CFblock, 
K, Q, CF, Glp , Ghp , Gbp, Gnotch, Gapf, Gubp, Gpeak, Gbshelf) 
= loop ~ si.bus(3) : par(i, 3, /(Treshold))
    with {
        ...
        loop(x, y, z) = 
        costrained(Dt1,Treshold,CFblock, K, Q, CF, Glp , Ghp , Gbp, Gnotch, Gapf, Gubp, Gpeak, Gbshelf, 
            (x + sigma * (y - x) *dt+(x_init)) 
        ),
        costrained(Dt2,Treshold,CFblock, K, Q, CF, Glp , Ghp , Gbp, Gnotch, Gapf, Gubp, Gpeak, Gbshelf, 
            (y + (rho * x - x * z - y) *dt+(y_init)) 
        ),
        costrained(Dt3,Treshold,CFblock, K, Q, CF, Glp , Ghp , Gbp, Gnotch, Gapf, Gubp, Gpeak, Gbshelf, 
...
routing(a,b,c) = (a*b)*(c*a)*c,(c*b)*(b*a)*a;
...
IR*x,IR*y,IR*z,
s,r,b,dt,
delEQ1, delEQ2, delEQ3, 
Treshold, CFblock, K, Q, CF, 
Glp , Ghp , Gbp, Gnotch, Gapf, Gubp, Gpeak, Gbshelf
) : routing;
